...
class SendNotifier extends Notifier<Map<String, SendSessionState>> {
  ...
  /// 返回本次发送的最终状态，供原生发送卡片准确展示“被拒绝”等结果。
  Future<SessionStatus> startSession({
    ...
    await _sendLoop(ref, sessionId, target, sendingFiles, onProgress);
    // 后台成功会主动从 state 移除 session，因此移除后即代表成功。
  ...
  Future<void> _sendLoop(
    ...
    final progressByFile = <String, double>{
      for (final file in files.values) file.file.id: file.status == FileStatus.skipped ? 1 : 0,
    };
    final totalBytes = files.values.fold<int>(0, (sum, file) => sum + file.file.size);

    // 用文件大小加权，多个文件并发上传时原生卡片也能显示准确的总进度。
    void reportProgress(String fileId, double progress) {
      progressByFile[fileId] = progress.clamp(0.0, 1.0).toDouble();
      if (totalBytes <= 0) {
        onProgress?.call(0);
        return;
      }
      ...
      for (final file in files.values) {
        completedBytes += file.file.size * (progressByFile[file.file.id] ?? 0);
      }
      onProgress?.call(
        (completedBytes / totalBytes).clamp(0.0, 1.0).toDouble(),
      );
    }
  ...
  Future<bool> sendFile({
    ...
    try {
      ...
      await for (final progress in taskResult.progress) {
        ...
        onProgress?.call(file.file.id, progress);
      }
      ...
      onProgress?.call(file.file.id, 1);
    } catch (e, st) {
