...
import 'package:localsend_app/util/native/macos_channel.dart' as macos;
...
class ReceiveController {
  ...
  Future<void> _prepareUploadHandler({
    ...
    if (quickSave) {
      ...
      if (checkPlatform([TargetPlatform.macOS])) {
        ...
        await macos.showReceivePanel(
          senderAlias: server.getState().session!.senderAlias,
          fileName: firstFile.fileName,
          fileCount: dto.files.length,
          totalSize:
              dto.files.values.fold<int>(0, (sum, file) => sum + file.size),
          destination: destinationDir,
        );

        final action = await macos.receivePanelActionStream.first;
        if (action == 'accept') {
          unawaited(
            macos.updateReceivePanel(
              status: 'receiving',
              progress: 0,
              currentFile: '准备接收',
            ),
  ...
  Future<void> _uploadHandler({
            ...
            if (checkPlatform([TargetPlatform.macOS])) {
              unawaited(
                macos.updateReceivePanel(
                  status: 'receiving',
                  progress: savedBytes / receivingFile.file.size,
                  currentFile: receivingFile.desiredName,
                ),
              );
            }
            if (checkPlatform([TargetPlatform.windows])) {
    ...
    if (allowedStates.contains(session.status) &&
      ...
      if (checkPlatform([TargetPlatform.macOS])) {
        final finishedFiles =
            session.files.values.where((file) => file.path != null).toList();
        ...
        unawaited(
          macos.updateReceivePanel(
            ...
            progress: 1,
            currentFile: firstPath == null
                ? null
                : File(firstPath).uri.pathSegments.last,
            openPath: firstPath,
