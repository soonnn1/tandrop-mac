...
class TanDropMacHomePage extends StatefulWidget {
  const TanDropMacHomePage({super.key});

  @override
  State<TanDropMacHomePage> createState() => _TanDropMacHomePageState();
}

class _TanDropMacHomePageState extends State<TanDropMacHomePage> {
  @override
  void initState() {
    super.initState();
  ...
  Future<void> _showUploadQr(BuildContext context) async {
    ...
    try {
      await showDialog<void>(
        context: context,
        builder: (_) => QrDialog(data: url, label: '扫码上传到此 Mac'),
      );
    } finally {
      if (restoreHttps) {
        await ref.notifier(serverProvider).restartServerFromSettings();
      ...
      children: [
        ...
        OutlinedButton(
          onPressed: widget.enabled
              ? () {
                  unawaited(_sendToDevice(context));
                }
              : null,
...
class _DeviceDetail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // InkWell 需要 Material 祖先，否则 macOS 运行时会出现 No Material widget found。
    return Material(
      color: Colors.transparent,
